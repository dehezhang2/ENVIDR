...
def blur_image(image: torch.Tensor, sigma=2):
    ''' blur an image with a gaussian kernel
    Args:
        image: [B, H, W, C]
        sigma: standard deviation of the gaussian kernel
    Return:
        blurred image: [B, H, W, C]
    '''
    kernel_size = 2 * sigma + 1
    image_blur = F.gaussian_blur(image.permute(0, 3, 1, 2), kernel_size, sigma)
    return image_blur.permute(0, 2, 3, 1)
...
def get_pose(device, theta, phi, radius=1):
    print("radius: ", radius)
    size = 1
    ...
    thetas = torch.ones(size, device=device) * theta
    phis = torch.ones(size, device=device) * phi
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if 'fl_x' in transform or 'fl_y' in transform:
            ...
            fl_y = intrinsics[1, 1] / downscale
            transform['cx'] = intrinsics[0, 2]
            transform['cy'] = intrinsics[1, 2]
        else:
    ...
    def collate(self, index):
        ...
        error_map = None if self.error_map is None or self.opt.error_map_start_iter > 0 else self.error_map[index]
        
        rays = get_rays(poses, self.intrinsics, self.H, self.W, self.num_rays, error_map, 
                        self.opt.patch_size, self.opt.center_crop, self.opt.center_crop_ratio)
        ...
        if self.images is not None:
            ...
            if self.opt.blur_sigma > 0:
                images = blur_image(images, self.opt.blur_sigma)
    ...
    def dataloader(self, test_ids=None, test_skip=1, batch_size=1):
        size = len(self.poses)
        if self.training and self.rand_pose > 0:
            size += size // self.rand_pose # index >= size means we use random pose.
        if test_ids is None:
            test_ids = list(range(0,size,test_skip))
        loader = DataLoader(test_ids, batch_size=batch_size, collate_fn=self.collate, shuffle=self.training, num_workers=0)
